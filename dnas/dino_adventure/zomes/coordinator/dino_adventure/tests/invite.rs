...
async fn invite_and_accept() {
    ...
    let bob_received_invite = Arc::new(AtomicBool::new(false));
    let bob_received_invite_clone = bob_received_invite.clone();
    let alice_agent_pub_key_clone = alice_cell_id.agent_pubkey().clone();
    bob_app_ws
        .on_signal(move |signal| {
            if let Signal::App { signal, .. } = signal {
                if let Ok(DinoAdventureSignal::AdventureInvite { sender }) =
                    signal.clone().into_inner().decode::<DinoAdventureSignal>()
                {
                    if sender == alice_agent_pub_key_clone {
                        bob_received_invite_clone.store(true, Ordering::SeqCst);
                    }
                }
            }
        })
    ...
    let alice_received_acceptance = Arc::new(AtomicBool::new(false));
    let alice_received_acceptance_clone = alice_received_acceptance.clone();
    let bob_agent_pub_key_clone = bob_cell_id.agent_pubkey().clone();
    alice_app_ws
        .on_signal(move |signal| {
            if let Signal::App { signal, .. } = signal {
                if let Ok(DinoAdventureSignal::InviteAcceptance { accepted_by }) =
                    signal.clone().into_inner().decode::<DinoAdventureSignal>()
                {
                    if accepted_by == bob_agent_pub_key_clone {
                        alice_received_acceptance_clone.store(true, Ordering::SeqCst);
                    }
                }
            }
        })
    ...
    let invite = AdventureInvite {
        to: bob_cell_id.agent_pubkey().clone(),
    };
    let _: () = alice_conductor
        .call(&alice_zome, "send_invite", invite)
        .await;

    // Wait for Bob to receive the invite signal
    let bob_timeout = Duration::from_secs(30);
    tokio::time::timeout(bob_timeout, async {
        while !bob_received_invite.load(Ordering::SeqCst) {
            tokio::time::sleep(Duration::from_millis(500)).await;
        }
    })
    .await
    .expect("Bob did not get an invite");

    // Bob accepts the adventure
    let acceptance = AdventureInviteAcceptance {
        to: alice_cell_id.agent_pubkey().clone(),
    };
    let _: () = bob_conductor
        .call(&bob_zome, "accept_invite", acceptance)
        .await;

    // Wait for Alice to receive the acceptance signal
    let alice_timeout = Duration::from_secs(30);
    tokio::time::timeout(alice_timeout, async {
        while !alice_received_acceptance.load(Ordering::SeqCst) {
            tokio::time::sleep(Duration::from_millis(500)).await;
        }
    })
    .await
    .expect("Alice did not get an acceptance");
}
