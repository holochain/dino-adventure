<script lang="ts">
  ...
  onMount(async () => {
    if (!currentRecord) {
      throw new Error(
        `The currentRecord input is required for the EditDino element`,
      );
    }
    if (!originalDinoHash) {
      throw new Error(
        `The originalDinoHash input is required for the EditDino element`,
      );
    }
